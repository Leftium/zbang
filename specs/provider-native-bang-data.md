...
Bootstrap data should be generated by the app after the client-side catalog generation works.

Initial implementation starts with no bootstrap data:

```text
First run -> empty bootstrap -> client downloads sources -> client generates full catalogs -> client persists catalogs
```
...
- `zbang.bootstrap.kagi.json`
- `zbang.bootstrap.duckduckgo.json`

Once checked in, bootstrap data should allow autocomplete/search to work immediately, then be replaced or extended after the client-generated full catalog is available.

Benefits:

- The same app code that generates full catalogs also produces bootstrap data.
- Bootstrap files are derived from real generated output, not a parallel build script.
- First implementation can ignore bootstrap and focus on correct client generation.
- Later releases can ship a small useful initial dataset without bundling the full catalog.
